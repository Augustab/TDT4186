...
int run (char input[MAX_INPUT_LEN]) {
    ...
    while(param != NULL) { // Iterating through paramaters
        if (!strcmp(param, "<") || !strcmp(param, ">")){ // End of parameters, break
            break;
        }
        printf("%s, ", param);
        params[i] = param;
    ...
    if (param != NULL && strcmp(param, "<") == 0) {// Setting redirection variables for input
        has_redirected_input = 1;
        in_file = strtok(NULL, delims);
        param = strtok(NULL, delims);
        printf("\033[0;33mRedirection-file-input: \033[0m%s \n", in_file);
    }
    if (param != NULL && strcmp(param, ">") == 0) {// Setting redirection variables for output
        has_redirected_output = 1;
        out_file = strtok(NULL, delims);
        param = strtok(NULL, delims);
        printf("\033[0;33mRedirection-file-output: \033[0m%s \n", out_file);
    }
    if (param != NULL && strcmp(param, "<") == 0) {// Setting redirecting variables, not actually doing redirecting here
    ...
    if (strcmp(command, "cd")==0  || strcmp(command, "CD")==0) { //internal command to change working directore
        if (chdir(params[0])!=0) {
            printf("Error CD-ing, %d\n", errno);
        }               
    }
    else if (strcmp(command, "exit")==0 || strcmp(command, "EXIT")==0) { //internal command to exit shell
        printf("Exiting...\n");
        exit(EXIT_SUCCESS);
    }
    //TASK B and C
    else { //external command -- creating child prosess for execution
        ...
        else if (pid == 0) //Child process
        {
            ...
            for (int c = 0; c < i; c++) { //making list argv out of parameters
                argv[c + 1] = params[c];
            }
            argv[i + 1] = NULL; //last element must be NULL

            if (in_file != NULL) { //redirect if inputfile is given
                int input = open(in_file, O_RDONLY); //reading in_file with open
                if (input == -1) { //if return-value of open is -1 an error occured
            ...
            if (out_file != NULL) { //redirect output if outputfile is given
                //printf("INNE HER\n");
                ...
                if (output == -1) { //if return-value of open is -1 an error occured
            ...
            if (execvp(command, argv) < 0) { //executing command with given parameters (returns negative number if error has occured)
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) { // Execute shell script
        FILE *stream;
        ...
        while ((readline = getline(&line, &lenght, stream)) != -1) {// Reading line for line from input-stream
